...
#df['Comment'] = df['Comment'].replace('Ð','-')
df = pd.read_csv('final_uganda.csv', encoding='ISO-8859-1')
...
def app():
    ...
    title = f'Comparison of Scores of Questions Within One Section'
    ...
    available_scores = df[(df['Module'] == module_selected) & (df['Part'] == part_selected)]['Score'].unique()
    ...
    if not filtered_data.empty:
        ...
        if plot_selected == 'Bar Plot':
            ...
            chart = alt.Chart(filtered_data).mark_bar().encode(
                y=alt.Y('Question1:N', sort=alt.EncodingSortField(field='Qn', order='ascending')),
                ...
                width=900,
                height=600,
                title=f'Bar Plot of Scores by Question within {module_selected1}: {part_selected}'
            )
            ...
            final_chart1 = alt.layer(pie, text1).properties(
                width=600,
                height=400,
                title=f'Pie Plot of Scores by Question within {module_selected1}: {part_selected}'
            ).configure_axis(
                labelFontSize=12,
                titleFontSize=14
            ).interactive()
            ...
            fig = px.line_polar(filtered_data, r='Score', theta='Question1', line_close=True,
                                ...
                                title=f'Radar Plot of Scores by Question within {module_selected1}: {part_selected}',
                                hover_data={
                                    'Question': True,
                                    'Score': True,  
                                    'Level': True,  
